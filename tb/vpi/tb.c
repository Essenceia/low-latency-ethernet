...
void tb_mac_rx(	
	...
	vpi_put_logic_uint32_t(h_debug_id_i, mac->id);
...
void tb_udp_data_rx(
	...
	if(udp_data){
		...
		fprintf(stderr,"ERROR: Empty fifo, no entry for upd data rx expected value\n");	
	}
	...
	vpi_put_logic_uint32_t(h_debug_id_o, id);	
}
